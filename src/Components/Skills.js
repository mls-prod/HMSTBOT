...
const Skills = () => {
    const {userLevels, prTeam, totalProfit, success, setTotalProfit, setSuccess, setUserLevels, balance, setBalance, refBonus, id, profitHour, setProfitHour } = useUser();
    ...
    const handleClickOutside = (event) => {
      ...
      const confirmUpgrade = async () => {
        ...
        if (index === 5 && userLevels[4] < 2) {
          setUpgradeStatus(
            <div className='w-full bg-modal rounded-[20px] py-[6px] flex items-center text-[#ffe253] px-4'>
              <span>Upgrade to level 2 of Technical Analysis required</span>
            </div>
          );
          setTimeout(() => {
            setUpgrading(false);
        ...
        if (index === 2 && userLevels[3] < 1) {
          setUpgradeStatus(
            <div className='w-full bg-modal rounded-[20px] py-[6px] flex items-center text-[#ffe253] px-4'>
              <span>Upgrade to level 1 of Awareness required</span>
            </div>
          );
          setTimeout(() => {
            setUpgrading(false);
        ...
        const updatedPrTeam = userData.prTeam ? [...userData.prTeam] : prTeam.map(() => ({
          title: '',
          level: 0,
          profit: 0,
          cost: 0,
        ...
        setTimeout(() => {
          ...
          setTotalProfit((prevTotalProfits) => prevTotalProfits.map((profit, i) => (i === index ? newTotalProfit : profit))); // Update total profits
...
{prTeam.map((team, index) => {
                ...
                const getLockMessage = () => {
                  if (index === 5 && userLevels[4] < 2) {
                    return 'T.Analysis lvl 2';
                  } else if (index === 2 && userLevels[3] < 1) {
                    return 'Awareness lvl 1';
                  }
                  return 'Locked';
                }

                return (
                  <div key={team.title} className='bg-cards w-[48%] pt-4 pb-2 rounded-[15px] [&:nth-child(2)]:!mt-0 text-[13px] flex flex-col justify-between'>
                    <button
                      ...
                      <div className='relative w-[60px] flex items-center pb-2 justify-center'>
                        <img src={team.icon} alt={team.title} className='w-[54px] h-[54px] rounded-full' />
                        {(index === 5 && userLevels[4] < 2) || (index === 2 && userLevels[3] < 1) ? (
                      ...
                      <div className='flex flex-col items-center justify-center'>
                        ...
                        <span className='text-nowrap text-[10px]'>Profit per hour</span>
                          ...
                          <span className={`${currentLevel === 0 ? 'text-secondary' : 'text-primary'} font-semibold text-[12px]`}>
                            {currentLevel === 0 ? (
                              <>
                                +{nextLevelData.profit}
                              </>
                            ) : (
                              <>
                            {profitsTotal}
...
<div className={`${upgrading ? 'visible top-1' : 'invisible top-[-12px] ease-out'} ease-in z-[60] transition-all duration-300 w-full flex absolute left-0 right-0 px-8`}>
          {upgradeStatus}
        </div>
  ...
  onClick={() => setOpenUpgrade(false)}
   className="flex items-center justify-center h-[32px] w-[32px] rounded-full bg-[#383838] absolute right-6 top-4 text-center font-medium text-[16px]"
            ...
            <div className="w-[80px] h-[80px] rounded-[25px] flex items-center justify-center">
              <img alt="claim" src={prTeam[selectedUpgrade.index].icon} className="w-[80px] rounded-full" />
            </div>
            ...
            <div className="flex flex-1 items-center justify-center space-x-1">
              ...
              <div className="font-bold text-[13px] flex items-center">+{formatNumber(selectedUpgrade.nextLevelData.profit)} 
                   </div>
            </div>
            </div>
            ...
            <div className="flex flex-1 items-center space-x-2 pt-3">
              ...
              <div className="font-bold text-[24px] flex items-center">{formatNumberCi(selectedUpgrade.nextLevelData.cost)} 
                   </div>
            </div>
          </div>
          ...
          {(balance + refBonus) >= selectedUpgrade.nextLevelData.cost ? (
           ...
           disabled={upgrading === true}
          className={`${((selectedUpgrade.index === 5 && userLevels[4] < 2) || (selectedUpgrade.index=== 2 && userLevels[3] < 1)) ? 'bg-[#42424264] text-[#979797]' : 'bg-btn4 text-[#000]'} w-full py-5 px-3 flex items-center justify-center text-center rounded-[12px] font-semibold text-[18px]`}
            >
